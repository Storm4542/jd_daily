...
function ReadCookie() {
    ...
    const EnvInfo4 = $nobyda.isJSBox ? "JD_Cookie4" : "CookieJD4";
    if (DeleteCookie) {
        if ($nobyda.read(EnvInfo) || $nobyda.read(EnvInfo2) || $nobyda.read(EnvInfo3) || $nobyda.read(EnvInfo4)) {
    ...
    if (Key || $nobyda.read(EnvInfo)) {
        if ($nobyda.isJSBox || $nobyda.isNode) {
            ...
            if (thirdKey) $nobyda.write(thirdKey, EnvInfo3);
            if (fourKey) $nobyda.write(thirdKey, EnvInfo4);
        }
        add = DualKey || thirdKey || fourKey ? true : false;
...
function third() {
    ...
    add = true;
    thirdAccount = false;
    if (DualKey || $nobyda.read($nobyda.isJSBox ? "JD_Cookie3" : "CookieJD3")) {
...
function fourt() {
    ...
    add = true;
    fourAccount = false;
    if (DualKey || $nobyda.read($nobyda.isJSBox ? "JD_Cookie4" : "CookieJD4")) {
        KEY = DualKey ? DualKey : $nobyda.read($nobyda.isJSBox ? "JD_Cookie4" : "CookieJD4");
        all();
    } else {
        $nobyda.time();
        $nobyda.done();
